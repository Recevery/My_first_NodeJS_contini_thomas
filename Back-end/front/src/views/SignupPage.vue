...
<script>
    ...
    export default {
        ...
        methods: {
            signUp() {
                axios.post("http://localhost:8081/auth/signup", { username: this.username_input, password: this.password_input, confirm_password: this.confirm_password_input
                }).then(user => { this.$store.dispatch('setUser', user.data.user); this.$router.push({name: "Home"})})
            }
        },
    }
</script>
